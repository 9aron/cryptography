...
def eval_rsa_encrypt(key_size, desc, form=BAR_FORMAT, n_t=N_TEST):
    rsa_setup_code = f"""
import random
import string
...
"""
    ...
    for _ in tqdm(range(n_t), desc=desc, bar_format=form):
        rsa_key_gen_time = timeit.timeit(stmt="encrypt(msg, pub_key)", setup=rsa_setup_code, number=1)
...
def eval_ecc_encrypt(curve, desc, form=BAR_FORMAT, n_t=N_TEST):
    ecc_setup_code = f"""
import random
import string
...
"""
    ...
    times = []
    for _ in tqdm(range(n_t), desc=desc, bar_format=form):
        ecc_key_gen_time = timeit.timeit(stmt="encrypt_ECC(msg, pub_key, c)", setup=ecc_setup_code, number=1)
...
for lvl, r_ks, e_ks in zip(SEC_LVL, RSA_KEYSIZE, EC):
    ...
    rsa_avg_t = eval_rsa_encrypt(int(r_ks), r_des)
    ecc_avg_t = eval_ecc_encrypt(e_ks, e_des)
