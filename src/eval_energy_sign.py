...
def eval_rsa_sign(sock, key_size, desc, form=BAR_FORMAT, n_t=N_TEST):
...
def eval_ecc_sign(sock, curve, desc, form=BAR_FORMAT, n_t=N_TEST):
    c = get_curve(curve)
...
for lvl, r_ks, e_ks in zip(SEC_LVL, RSA_KEYSIZE, EC):
    ...
    rsa_avg_t = eval_rsa_sign(sock, int(r_ks), r_des)
    ecc_avg_t = eval_ecc_sign(sock, e_ks, e_des)
