...
def eval_rsa_sign(sock, msg, key_size, desc, form=BAR_FORMAT, n_t=N_TEST):
...
def eval_ecc_sign(sock, msg, curve, desc, form=BAR_FORMAT, n_t=N_TEST):
    c = get_curve(curve)
    ...
    for _ in tqdm(range(n_t), desc=desc, bar_format=form):
        ...
        generate_ecc_sig(priv_key, msg, c)
...
for lvl, r_ks, e_ks in zip(SEC_LVL, RSA_KEYSIZE, EC):
    print(f"\n[+] security level {lvl}")
    r_des = f"[+] sign in {r_ks} bit rsa key"
    e_des = f"[+] sign in {e_ks[4:7]} bit ecc key"

    rsa_avg_t = eval_rsa_sign(sock, msg, int(r_ks), r_des)
    ecc_avg_t = eval_ecc_sign(sock, msg, e_ks, e_des)
